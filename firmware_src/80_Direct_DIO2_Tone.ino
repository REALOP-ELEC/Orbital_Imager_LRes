...
SoftwareSerial mySer(PA3, PA2);
SX127XLT LoRa;                                                 //create a library class instance called LoRa


void loop()
{
  mySer.print(TXpower);                                       //print the transmit power defined
  mySer.print(F("dBm "));
  mySer.println(F("PlayTones> "));
  mySer.println();
...
void setup()
{
  ...
  mySer.begin(9600);
  mySer.println();
  mySer.print(F(__TIME__));
  mySer.print(F(" "));
  mySer.println(F(__DATE__));
  mySer.println(F(Program_Version));
  mySer.println();
  mySer.println(F("80_Direct_DIO2_Tone Starting"));
  ...
  {
    mySer.println(F("LoRa Device found"));
    led_Flash(2, 125);                                   //two further quick LED flashes to indicate device found
    delay(1000);
  }
  else
  {
    mySer.println(F("No device responding"));
    while (1)
    {
  ...
  mySer.print(F("Tone Transmitter ready"));
  mySer.println();
}
